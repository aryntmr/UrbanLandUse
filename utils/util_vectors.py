...
def load_shape(place_shapefile):
    # #print("printing the type of the place_Shapefile",type(place_shapefile))
    # c = fiona.open(place_shapefile)
    # pol = c.next()
    # #print(c)
    # shape = {}
    # #print("printing the keys for shapefile")
    # #print(len(pol.keys()),pol.keys())
    # #print(pol)
    # shape['type'] = pol.geometry.type
    ...
    with fiona.open(place_shapefile) as c:
        ...
        shape['type'] = pol['geometry']['type']  # Access via the geometry dictionary
        shape['properties'] = dict(pol['properties'])  # Convert properties to dictionary
        shape['geometry'] = {}
        shape['geometry']['type'] = pol['geometry']['type']
        shape['geometry']['coordinates'] = []
        ...
        if len(pol['geometry']['coordinates']) > 1:
            ...
            for p in range(len(pol['geometry']['coordinates'])):
                ...
                if pn > pn_max:
                    ...
                    pn_max = pn
                print(p, pn, p_argmax, pn_max)
            # Set largest polygon as the main polygon
            polygon = pol['geometry']['coordinates'][p_argmax]
        else:
            print('Simple Polygon')
            polygon = pol['geometry']['coordinates']

        # Construct coordinates for the polygon
        ring = []
        for x, y in polygon[0]:
            ring.append((x, y))
        
        # Ensure the coordinates are correctly nested
        shape['geometry']['coordinates'] = [ring]
        
        # Initialize bounding box variables
        xmin = min(x for x, y in ring)
        xmax = max(x for x, y in ring)
        ymin = min(y for x, y in ring)
        ymax = max(y for x, y in ring)
        shape['bbox'] = [xmin, ymin, xmax, ymax]
